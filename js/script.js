...
const checkGameOver = () => {
  if (
    (playerCat.position.x + playerCat.width - 2 * playerCat.offsetImage >=
      playerRat.position.x + playerRat.offsetImage &&
      playerCat.position.x + playerCat.width - 2 * playerCat.offsetImage <=
        playerRat.position.x + playerRat.width - playerRat.offsetImage &&
      playerCat.position.y + playerCat.width <=
        playerRat.position.y + playerRat.width &&
      playerCat.position.y + playerCat.width >=
        playerRat.position.y + playerRat.width - (1 / 3) * playerRat.width &&
...
const gameLoop = () => {
  ...
  if (
    playerCat.direction.left == true &&
    playerCat.position.x >= canvas.offsetLeft - playerCat.offsetImage
  )
    playerCat.moveLeft();
  if (
    playerCat.direction.right == true &&
    playerCat.position.x <=
      canvas.width - playerCat.offsetImage - canvas.offsetLeft
  )
    playerCat.moveRight();
  if (
    playerCat.direction.jump == true &&
    playerCat.position.x >= 0 &&
    playerCat.position.x <= canvas.width - playerCat.width
  )
    playerCat.jump();

  if (
    playerRat.direction.left == true &&
    playerRat.position.x >= canvas.offsetLeft - playerRat.offsetImage
  )
    playerRat.moveLeft();
  if (
    playerRat.direction.right == true &&
    playerRat.position.x <=
      canvas.width + playerRat.offsetImage - canvas.offsetLeft
  )
    playerRat.moveRight();
  if (
    playerRat.direction.jump == true &&
    playerRat.position.x >= 0 &&
    playerRat.position.x <= canvas.width - playerRat.width
  )
    playerRat.jump();
